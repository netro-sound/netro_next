...
export function GridPlaylists({ playlists }: Props) {
  const [changeTrack, setQueue] = usePlayerStore((state) => [
    state.changeTrack,
    state.setQueue,
  ]);
  const [checkedTracks, setCheckedTracks] = useState<IPlaylist[]>([]);
  const { handleContextMenu, points, show, item } =
    useContextMenu<IPlaylist[]>();
    ...
    <>
      ...
      <GridWrapper smColumns={2} mdColumns={8} className="gap-x-4 gap-y-8">
        {playlists.map((playlist, index) => {
          return (
            <Link href={`/playlists/${playlist.spotify_id}`} key={index}>
              <div
                // onClick={(ev) => handleTrackClick(ev, track)}
                aria-label="column"
                className={classNames(
                  'group cursor-pointer w-full gap-4 px-2',
                  checkedTracks.includes(playlist) && 'bg-base-200'
                )}
              ...
              >
                ...
                <div className="overflow-x-clip w-full">
                  <p className="truncate" title={playlist.name}>
                    {playlist.name}
                  </p>
                </div>
